...
export const MultiSphereDemo: React.FC<MultiSphereDemoProps> = ({
  ...
  // Track mouse position for tooltip without interfering with magnetic effects
  useEffect(() => {
  ...
  const Sphere: React.FC<{ sphere: SphereData }> = ({ sphere }) => {
    ...
    return (
      <div
        ...
        style={{
          ...
          backgroundColor: generateSphereColor(sphere.id, spheres.length),
        }}
      />
    );
  };
    ...
    <div className={styles.demoContent}>
      <div className={styles.infoPanel}>
        ...
        <div className={styles.infoPanelStats}>
          <div>Active spheres: {spheres.length}</div>
      ...
      <div className={styles.sphereField}>
        {spheres.map((sphere) => (
          <Sphere key={sphere.id} sphere={sphere} />
        ))}
      </div>
